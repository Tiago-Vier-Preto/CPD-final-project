...
INTERNAL_NODE = 0
LEAF_NODE = 1
## every key will smaller than this number
...
class bplus_tree:
    ...
    def borrow_or_merge(self, key, child, parent, idx):
        ...
        if self._can_borrow(left_child):
            ...
            self._shift_right(child.keys, 0, idx)
            if self.is_leaf(child):
                self._shift_right(child.values, 0, idx)
            else:
                self._shift_right(child.childs, 0, idx)
    ...
    def _init_tree(self, path):
        self.file = accessfile(path + "/" + self.default_db_name, "wb+")
        offset = 0
    ...
    def _init_read_bplustree_conf(self, path):
        self.file = accessfile(path + "/" + self.default_db_name)
